...
# --- CV scaling ------------------------------------------------------------
Y_nom = np.array([64.0, 7.0, 9.0])          # nominal physical CVs
...
U_nom = np.array([450., 100., 2., 950.])    # operating point (physical)

u_range = u_max - u_min
def scale_mv(u_phys):   return  2*(u_phys - u_min)/u_range - 1.0      # → [-1,1]
def descale_mv(u_scaled): return u_min + 0.5*u_range*(u_scaled + 1.0)

u_scaled_nom = scale_mv(U_nom)              # nominal in scaled units

# --- Set‑point in *scaled* coordinates -------------------------------------
...
# ============================================================================
# 3) PI gains (work in scaled domain)
# ============================================================================
# Kp_base = np.array([1.5, 0.5, 0.05, 0.1])
# Ki_base = np.array([0.25, 0.25, 0.0025, 0.01])
Kp_base = np.array([1.5, 0.1, 0.05, 0.1])
Ki_base = np.array([0.25, 0.05, 0.05, 0.01])
# ============================================================================
# 4) Simulation
# ============================================================================
records = []
...
for run in range(N_RUNS):
    ...
    # --- random initial state & random gain multipliers --------------------
    x = np.random.uniform(-0.1, 0.1, size=10)


    # ── element‑specific uncertainty bands  (lo, hi) per gain ──────────
    Kp_band = [ (0.8, 0.3),   # Kp1  ±20 %
                (0.5, 0.8),   # Kp2  −50 % … +50 %
                (0.70, 1.30),   # Kp3  ±30 %
                (0.90, 1.10) ]  # Kp4  ±10 %
    
    Ki_band = [ (0.08, 0.3),   # Ki1
                (0.1, 0.8),   # Ki2
                (0.9, 1.40),   # Ki3
                (1, 1.10) ]  # Ki4
    
    # ── draw independent multipliers for each gain ─────────────────────
    Kp_mult = np.array([np.random.uniform(lo, hi) for (lo, hi) in Kp_band])
    Ki_mult = np.array([np.random.uniform(lo, hi) for (lo, hi) in Ki_band])
    
    # ── scaled gains for this run ───────────────────────────────────────
    Kp = Kp_base * Kp_mult
    Ki = Ki_base * Ki_mult
    int_term = np.zeros(4)

    u_scaled = u_scaled_nom.copy()          # controller works on scaled MVs
    prev_err_metric = None

    for k in range(N_STEPS):
        # ------------------------------------------------------------------
        # 1) Plant → get scaled CV measurement (with ±3 % multiplicative noise)
        # ------------------------------------------------------------------
        y_dev_phys  = C @ x                        # deviation from nominal (phys)
        noise       = np.random.uniform(0.97, 1.03, size=3)
        y_phys      = (Y_nom + y_dev_phys) * noise
        y_scaled    = scale_cv(y_phys)             # controller sees scaled value

        # ------------------------------------------------------------------
        # 2) PI control law in *scaled* space
        # ------------------------------------------------------------------
        err_scaled  = Ysp_scaled - y_scaled
        loop_err    = np.array([*err_scaled, err_scaled[0]])  # size 4

        P_term      = Kp * loop_err
        u_unsat     = u_scaled_nom + P_term + int_term
        u_sat       = np.clip(u_unsat, -1.0, 1.0)

        # Anti‑wind‑up (still scaled)
        for i in range(4):
            sat_lo = u_unsat[i] < -1.0 and loop_err[i] < 0
            sat_hi = u_unsat[i] >  1.0 and loop_err[i] > 0
            if not (sat_lo or sat_hi):
                int_term[i] += Ki[i] * loop_err[i]

        u_scaled = np.clip(u_scaled_nom + P_term + int_term, -1.0, 1.0)

        # ------------------------------------------------------------------
        # 3) Plant state update  (needs *physical* Δu)
        # ------------------------------------------------------------------
        u_phys      = descale_mv(u_scaled)
        delta_u_phys = u_phys - U_nom
        x = A @ x + B @ delta_u_phys

        # ------------------------------------------------------------------
        # 4) Reward (based on scaled error)
        # ------------------------------------------------------------------
...
# ============================================================================
# 5) Save CSV (scaled values only) and quick overlay plot
# ============================================================================
df       = pd.DataFrame.from_records(records)
CSV_PATH = "paper_machine_batch_log_scaled.csv"
