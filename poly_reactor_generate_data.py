...
def pid_controller(P_s, T_s, int_P, int_T, setpts, dt,
                   Kp_P, Ki_P, Kp_T, Ki_T):
    err_P, err_T = setpts["P"] - P_s, setpts["T_s"] - T_s
    I_raw = Kp_P*err_P + Ki_P*int_P
    u_I   = float(np.clip(I_raw, 0, 1))
    if u_I == I_raw: int_P += err_P*dt

    Tc_raw = Kp_T*err_T + Ki_T*int_T+0.535
    u_Tc   = float(np.clip(Tc_raw, 0, 1))
    if u_Tc == Tc_raw: int_T += err_T*dt

    return u_I, u_Tc, int_P, int_T
...
def simulate_many_runs(n_runs: int = 20,
    ...
    base = dict(Kp_P=2.6*0.8, Ki_P=0.0002*0.8,
                Kp_T=0.03*0.8, Ki_T=0.000095*0.8)
    ...
    for run in range(n_runs):
        ...
        for _ in range(n_steps):
            ...
            # --- PID control on noisy measurements -------------------------
            u_I, u_Tc, int_P, int_T = pid_controller(
                y_meas[3], y_meas[4], int_P, int_T,
                setpts, dt, Kp_P, Ki_P, Kp_T, Ki_T
            )
...
if __name__ == "__main__":
    df = simulate_many_runs(n_runs=100,            # ← choose how many runs
                            t_total=100*3600,      # 24 h each (optional)
                            dt=30*60,             # 30-min sample
                            seed=123)
    df.to_csv("multi_run_dataset.csv", index=False)
